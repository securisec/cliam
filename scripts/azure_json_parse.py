...
path = Path('temp/azure-rest-api-specs/specification/web/resource-manager/Microsoft.Web/stable/2021-03-01/WebApps.json')
VERSION = '2021-03-01'
...
with path.resolve().open('r') as f:
    ...
    for endpoint in data['paths'].keys():
        for method, resource in data['paths'][endpoint].items():
            ...
            if method == 'get':
                if len(params) == 0:
                    hold.append(process_get(endpoint, resource))
