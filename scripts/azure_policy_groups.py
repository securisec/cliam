...
def process_get(path, oper, method, VERSION):
    return f"""{{
    ...
    try:
        ...
        for dirs in rdirs:
            ...
            for resource_path in getPolicies(RESOURCE, SPECIFICATION, VERSION):
                ...
                var_name = 'policy.' + RESOURCE.replace(".", "_") + f"_{resource}" + ','
                hold[SPECIFICATION].append(var_name)
...
template = """package azure

import "github.com/securisec/cliam/azure/policy"

// GroupedPolicies grouped policies
var GroupedPolicies = map[string][]map[string]policy.Policy{{
    {policies}
}}
"""

hold = OrderedDict(sorted(hold.items(), key=lambda x: x[0]))
